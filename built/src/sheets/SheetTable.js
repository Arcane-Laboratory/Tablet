...
class SheetTable extends Table_1.Table {
   ...
   */
  constructor(
    name, // also used as spreadsheet tab name
    spreadsheetInfo,
    exampleEntry
  ) {
    super(name)
    ...
    this.findRowIndexById = (id) => {
      const index = this.rows.findIndex(
        (row) => (0, sheetsUtil_1.parseVal)(row.id).toString() == id.toString()
      )
      return index
    }
  ...
  async crupdate(entry, changes = false) {
    if (!entry.id) entry.id == (0, crypto_1.randomUUID)()
  ...
  async load() {
    this.spreadsheet = await (0, sheetsUtil_1.loadSpreadsheet)(
