...
class Table {
    ...
    async clone(targetTable, verbose) {
        ...
        if (verbose)
            console.log(` Tablet: Cloning ${this.numEntries()} entries from ${this.name} into ${targetTable.name}`);
    ...
    static getSummary(verbose) {
        ...
        Table.all.forEach((table) => {
            ...
            info.push(` ${table.name} [${Object.getPrototypeOf(table).constructor.name}]`);
