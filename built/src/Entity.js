...
class Entity {
    ...
    static registerEntity(table, loadFactory) {
        if (table == undefined)
            throw new Error(`TABLET_ENTITY.registerEntity: ${this.name} table can't be undefined`);
        if (loadFactory == undefined)
            throw new Error(`TABLET_ENTITY.registerEntity: ${this.name} loadFactory can't be undefined`);
    ...
    static async fetchAll() {
        ...
        const entityPromises = allRecords.map(async (record) => {
            const entity = await Entity.build(record, this).catch((err) => console.log(err));
            if (entity)
    ...
    static async filterEntity(filterFn) {
        ...
        const newEntities = await Promise.all(records.map(async (record) => {
            ...
            if (foundEntity)
                ...
                return Entity.build(record, this);
        }));
        return newEntities;
    }
    /**
